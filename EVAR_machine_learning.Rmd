...
Pipeline: Split -> Impute -> One-hot Encode -> Scale
...
dummyEVAR <- dummyVars(" ~ .", data = EVAR_preprocess, fullRank=F, dummy_na = TRUE)
train_onehot <- data.frame(predict(dummyEVAR, newdata = train_imputed))
test_onehot <- data.frame(predict(dummyEVAR, newdata = test_imputed))
```
