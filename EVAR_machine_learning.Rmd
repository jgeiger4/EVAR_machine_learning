...
levels(train_sacreg$reintervention.Yes) <- c("Alive", "Dead")
...
SVMROC <- roc(test_sacreg$reintervention.Yes, SVMpred$Dead)
...
```{r}
sac_reg_shap <- s
sacreg_sv <- shapviz(sac_reg_shap)
survival_sv <- shapviz(survival_shap)
sv_importance(sacreg_sv$X1, kind = "beeswarm", max_display = 10)
sv_importance(survival_sv$Dead, kind = "no", max_display = 10)
