...
pacman::p_load(pacman, 
               ...
               # extras
               forcats, janitor, explore, caTools, Metrics, VIM, car, kernelshap, shapviz)
```
...
# predict on test set
RFpred <- predict(RF, newdata = test_sacreg %>% select(-reintervention.Yes), # use the test set
                         type = "prob")

RFROC <- roc(test_sacreg$reintervention.Yes, RFpred$X1)
...
# predict on test set
XGBpred <- predict(XGB, newdata = test_sacreg %>% select(-reintervention.Yes), # use the test set
                         type = "prob")

XGBROC <- roc(test_sacreg$reintervention.Yes, XGBpred$X1)
...
s <- kernelshap(sac_regression_XGB, 
                X = X_explain, 
                bg_X = X_explain[sample(nrow(X_explain), 150), ]) 
...
sv_importance(survival_sv, kind = "beeswarm", max_display = 10)

```

```{r}
sv_dependence(sv, X_explain)
