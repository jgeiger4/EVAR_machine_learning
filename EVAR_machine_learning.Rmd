...
```{r}
svm_model <- train(survival_1yr_censor.1 ~ sii + nlr + insurance.Public + maximum_aaa_diameter + prior_cabg,
            data = train_sacreg,
            method = "svmRadial",
            trControl = repeatedcv_trControl,
            tuneGrid = SVMgrid,
...
svm_model_pred <- predict(svm_model, newdata = test_sacreg %>% select(-survival_1yr_censor.1), # use the test set
                         type = "prob")
svm_modelROC <- roc(test_sacreg$survival_1yr_censor.1, svm_model_pred$X1)
...
sv_importance(survival_sv$Dead, kind = "bar", max_display = 10, show_numbers = TRUE, format_fun = format_max)
```
...
```{r}
survival1yr_SMV <- SVM
survival1yr_SMV_ROC <- SVMROC

```

```{r}
sac_regression_XGB <- XGB
sac_regression_XGB_ROC <- XGBROC
```
