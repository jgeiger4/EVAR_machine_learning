...
pacman::p_load(pacman, 
               ...
               # extras
               forcats, janitor, explore, caTools, Metrics, VIM, car, kernelshap, shapviz, DMwR, doFuture)
```
...
cvd, prior_chf, copd, diabetes, smoking, 
...
# dysrhy
EVAR_preprocess <- EVAR_preprocess %>%
  mutate(atrial_dysrhy = case_when(dysrhy == "Yes, atrial" ~ "atrial",
                                   .default = "No")) %>%
  select(-dysrhy)
...
```{r}
post_op_features <- c("iv_bp_support_post_op",
                      "postop_complications",
                      "discharge_status",
                      "discharge_asa",
                      "discharge_p2y12_antagonist",
                      "discharge_statin",
                      "discharge_beta_blocker",
                      "discharge_ace_inhibitor_arb",
                      "discharge_anticoagulant"
                      )
EVAR_preprocess <- EVAR_preprocess[,!(names(EVAR_preprocess) %in% post_op_features)]
...
functional_status, cad_symptoms, 
...
ordinal_factors <- c("functional_status", "cad_symptoms", "cvd", "cad_symptoms", "prior_chf", "copd", "diabetes", "dialysis", "smoking", "prior_cabg", "prior_pci", "prior_cea_cas", "prior_aneurysm_repair", "prior_major_amp", "stress_test", "ejection_fraction", "aorta_neck_angle", "neck_aaa_angle", "iliac_aneurysm")
...
other_outcome_vars <- c("sac_regression_12m_cat.Regression", 
                         "survival_5yr_censor.1",
                         "reintervention.Yes"
                        )
...
# train model
LR <- train(survival_1yr_censor.1 ~ .,
            data = train_sacreg,
               method = "glm",
               family = "binomial",
               trControl = repeatedcv_trControl,
               metric = "ROC",
            weights = class_weights# Increase max iterations
               )

# Make predictions on the test set
LRpred <- predict(LR,
                  s = LR$bestTune$lambda, # choose the best lambda
                  newdata = test_sacreg %>% select(-survival_1yr_censor.1), # use the test set
                  type = "prob")

LRROC <- roc(test_sacreg$survival_1yr_censor.1, LRpred$X1)
...
# Prepare your data
LassoX <- model.matrix(reintervention.Yes ~ ., data = train_sacreg)
Lassoy <- train_sacreg$reintervention.Yes

# Fit the logistic regression model with Lasso regularization
lasso_model <- cv.glmnet(LassoX, Lassoy, family = "binomial", alpha = 1)

# Choose the optimal lambda (regularization parameter) using cross-validation
best_lambda <- lasso_model$lambda.min

# Refit the model using the selected lambda
lasso_model_final <- glmnet(LassoX, Lassoy, family = "binomial", alpha = 1, lambda = best_lambda)
...
cv0 <- cv.glmnet(LassoX, Lassoy, alpha = 0, lambda = lambdas_to_try,
                 standardize = TRUE, nfolds = 5, type.measure = "mse", family = "binomial")
cv03 <- cv.glmnet(LassoX, Lassoy, alpha = 0.8, lambda = lambdas_to_try,
                  standardize = TRUE, nfolds = 5, type.measure = "mse", family = "binomial")
cv07 <- cv.glmnet(LassoX, Lassoy, alpha = 0.9, lambda = lambdas_to_try,
                  standardize = TRUE, nfolds = 5, type.measure = "mse", family = "binomial")
...
par(mfrow=c(2,2))
plot(cv0);plot(cv03);plot(cv07);plot(cv1)

# Compare the solution paths
par(mfrow=c(2,2))
plot(glmnet(LassoX, Lassoy, alpha = 0, family = "binomial"),xvar="lambda");
...
# train RF model
XGB <- train(survival_1yr_censor.1 ~ .,
            data = train_sacreg,
            method = "xgbTree",
            trControl = repeatedcv_trControl,
            # tuneGrid = XGBgrid,
...
# predict on test set
XGBpred <- predict(XGB, newdata = test_sacreg %>% select(-survival_1yr_censor.1), # use the test set
                         type = "prob")

XGBROC <- roc(test_sacreg$survival_1yr_censor.1, XGBpred$X1)
...
NBgrid <- expand.grid(
  ...
  adjust = c(0, 0.5, 1.0))
# train RF model
NB <- train(survival_1yr_censor.1 ~ .,
            data = train_sacreg,
            method = "naive_bayes",
            trControl = repeatedcv_trControl,
            tuneGrid = NBgrid,
...
# predict on test set
NBpred <- predict(NB, newdata = test_sacreg %>% select(-survival_1yr_censor.1), # use the test set
                         type = "prob")

NBROC <- roc(test_sacreg$survival_1yr_censor.1, NBpred$X1)
...
levels(train_sacreg$survival_1yr_censor.1) <- c("Alive", "Dead")
class_weights <- ifelse(train_sacreg$survival_1yr_censor.1 == "Dead", 7.69, 0.53)
...
SVMgrid <- expand.grid(sigma = c(0.01, 0.05, 0.1, 1),
                       C = c(0.01, 0.05, 0.1, 1))
# train RF model
SVM <- train(survival_1yr_censor.1 ~ .,
            data = train_sacreg,
            method = "svmRadial",
            trControl = repeatedcv_trControl,
            tuneGrid = SVMgrid,
...
# predict on test set
SVMpred <- predict(SVM, newdata = test_sacreg %>% select(-survival_1yr_censor.1), # use the test set
                         type = "prob")

SVMROC <- roc(test_sacreg$survival_1yr_censor.1, SVMpred$Dead)
...
```{r}
X_explain <- train_sacreg %>%
  select(-survival_1yr_censor.1)

# take subsample as bg_X if data has >500 rows or so
kernel_s <- kernelshap(SVM, 
                X = X_explain,
                bg_X = X_explain) 
...
```{r}
survival_sv <- shapviz(s)
shap_importance <- sv_importance(survival_sv$Dead, kind = "bar", max_display = 10, show_numbers = TRUE, format_fun = format_max)

shap_importance_df <- as.data.frame(shap_importance$data)

shap_importance_df <- shap_importance_df %>%
  mutate(feature = fct_recode(feature,
                              "Atrial Dysrhythmia" = "atrial_dysrhyNo",
                              "Preop P2Y12 Antagonist" = "pre_op_p2y12_antagonist.Yes",
                              "SII" = "sii",
                              "Prior CABG" = "prior_cabg",
                              "Neck AAA Angle" = "neck_aaa_angle",
                              "Maximum AAA diameter" = "maximum_aaa_diameter",
                              "COPD" = "copd",
                              "Years quit smoking" = "years_quit_smoking",
                              "Unfit for Open AAA Repair" = "unfit_for_open_aaa_repair.Yes",
                              "Smoking" = "smoking"))



shap_importance_barplot <- ggplot(shap_importance_df, aes(x = feature, y = value)) +
  geom_bar(stat = "identity", fill = "#00205B") +
  geom_text(aes(label = sprintf("%.5f", y = value)), 
            vjust = 0.5, hjust = 1.1, color = "white", family = "Helvetica Neue", fontface = "bold") +  # Add text inside bars
  coord_flip() +
  labs(x = "", y = "mean(|SHAP value|)") +  # Optionally, set axis labels
  theme_bw() +
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        axis.text = element_text(size = 12, family = "Helvetica Neue", face = "bold", color = "black"),  # Adjust font size and family of axis text
        axis.title = element_text(size = 12, family = "Helvetica Neue", face = "bold"))
    
shap_importance_barplot

```

```{r}
ggsave("SHAP_importance_plot.png",
       plot = shap_importance_barplot, dpi = 300, height = 5, width = 8)
```


```{r}
sv_dependence(survival_sv$Dead, v = c("atrial_dysrhyNo"))
```
...
plot_roc_comparison <- function(roc1, roc2, 
  ...
  plot <- ggplot() +
    geom_line(data = roc1_df, aes(x = FPR, y = TPR), 
              color = roc1_color, linetype = "solid", linewidth = 1.5) +
    
    geom_ribbon(data = dat.ci1, 
                aes(x = 1 - x, ymin = lower, ymax = upper), fill = "orange", alpha= 0.2) +
    
    geom_line(data = roc2_df, aes(x = FPR, y = TPR), 
              color = roc2_color, linetype = "solid", linewidth = 1.5) +
    
    geom_ribbon(data = dat.ci2, 
                aes(x = 1 - x, ymin = lower, ymax = upper), fill = "steelblue", alpha= 0.2) +
    ...
    annotate("text", x = 0.5, y = 0.13, 
             label = paste("1-year mortality AUC:", round(auc1, 3)), 
             color = roc1_color, size = 6, hjust = 0, vjust = 0) +
    
    annotate("text", x = 0.5, y = 0.05, 
             label = paste("Sac Regression AUC:", round(auc2, 3)), 
             color = roc2_color, size = 6, hjust = 0, vjust = 0)
...
<<<<<<< HEAD
```{r}
resultsplot <- plot_roc_comparison(survival1yr_SMV_ROC, sac_regression_XGB_ROC, title = "Receiver Operator Characteristic Curve")
ggsave("ml_result.png", plot = resultsplot, dpi = 300, height = 6, width = 7.5)
```
...
pld <- rbind(cbind(Alpha = 0, min = cv0$lambda.min, se1 = cv0$lambda.1se),
      cbind(Alpha = 0.2, min = cv02$lambda.min, se1 = cv02$lambda.1se),
      cbind(Alpha = 0.4, min = cv04$lambda.min, se1 = cv04$lambda.1se),
      cbind(Alpha = 0.6, min = cv06$lambda.min, se1 = cv06$lambda.1se),
      cbind(Alpha = 0.8, min = cv08$lambda.min, se1 = cv08$lambda.1se),
      cbind(Alpha = 0.9, min = cv09$lambda.min, se1 = cv09$lambda.1se),
...
>>>>>>> 192992198ed71e2980d07f83cfad5ef617eff561
