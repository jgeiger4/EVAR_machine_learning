...
```{r reorder-factors}
# dysrhy
EVAR_preprocess$dysrhy <- fct_relevel(EVAR_preprocess$dysrhy, "No", "Yes, atrial", "Other")
#cvd
...
EVAR_preprocess <- EVAR_preprocess %>%
  ...
  mutate(number_right = as.numeric(as.character(number_right)))

# number_left
EVAR_preprocess <- EVAR_preprocess %>%
  mutate(number_left = fct_relevel(number_left,"None", "1", "2", "3")) %>%
  mutate(number_left = fct_recode(number_left, "0" = "None")) %>%
  mutate(number_left = as.numeric(as.character(number_left)))
...
EVAR_preprocess$right_iliac_endpoint <- fct_relevel(EVAR_preprocess$right_iliac_endpoint, "None","Common", "External, Intended", "External, Unintended")

# left_iliac_endpoint
EVAR_preprocess$left_iliac_endpoint <- fct_relevel(EVAR_preprocess$left_iliac_endpoint, "None","Common", "External, Intended", "External, Unintended")
...
Pipeline: Split -> Impute -> One-hot Encode -> Scale -> Feature Selection
